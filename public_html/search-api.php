...
function get_languages_from_db(&$lang){
  global $opuspub;
...
function get_trg_languages_from_db($lang,$srclang,&$trgs){
  global $opuspub;
...
function get_trg_languages($lang,$srclang,&$trgs){
    global $OPUSAPI;
    $url = $OPUSAPI.'/?languages=True&source='.$lang;
    $lines_array=explode("\n",getHTML($url,10));
    // turn array into one variable
    $string=implode('',$lines_array);
    $json = json_decode($string, true);
    foreach ($json['languages'] as $l){
        $trgs[$l] = $srclang[$l];
        // $trgs[$l] = locale_get_display_language($l, 'en');
    }
    return true;
}
